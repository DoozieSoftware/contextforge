...
import { offlineBreakdown } from "./offline.js";
...
export async function runBreakdown(opts: BreakdownOptions, ctx: CommandContext): Promise<CommandResult> {
  const inputAbs = path.isAbsolute(opts.inputFile) ? opts.inputFile : path.join(ctx.cwd, opts.inputFile);
  if (!fs.existsSync(inputAbs)) {
    throw new Error(`Requirement file not found: ${opts.inputFile}`);
  }
  ...
  if (!ctx.offline) {
    const plannerOut = await runPlanner({
      goal: `Pick up to ${opts.maxFiles ?? 5} project files that give relevant context to break down the requirement.`,
      systemPrompt: loadPromptTemplate("breakdown"),
      candidateSummary: `Requirement input: ${path.relative(ctx.cwd, inputAbs)}\n\nRequirement text:\n${inputText.slice(0, 4000)}`,
      root: ctx.root,
      memory: ctx.memory,
  ...
  if (!ctx.offline) {
    const writerOut = await runWriter({
      command: "breakdown",
      promptBody: loadPromptTemplate("breakdown"),
      contextPackage:
        `Requirement input: ${path.relative(ctx.cwd, inputAbs)}\n\nRequirement text:\n${inputText}\n\n` +
        packageToMarkdown(pkg, true),
      userGoal: `Break down the requirement. Planner notes: ${planNotes}`,
      appConfig: ctx.appConfig,
      provider: ctx.provider,
      stats: ctx.stats,
    });
    ...
    body = offlineBreakdown(inputText, ctx.root, ctx.memory);
  }
  ...
  return {
    ...
    stats: ctx.stats,
    report,
    title: `Breakdown: ${path.basename(inputAbs)}`,
